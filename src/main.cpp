...
WebSocketsServer webSocket(81); // WS port

bool relayState[4] = {0,0,0,0};

// ===== WEBPAGE HYBRID VOICE CONTROL =====
String webpage(){
return R"rawliteral(
<!DOCTYPE html>
<html>
...
function startVoice() {
  ...
  recognition.onresult = function(event) {
    ...
    // Bahasa Indonesia
    if(command.includes("nyalakan d2 laser kuning") || command.includes("hidupkan relay 1")) relayId=1,state="ON";
...
<div class="card">Voice Control<br><br>
<button id="voiceBtn" class="btn off" onclick="startVoice()">🎤 Bicara</button>
</div>
...
void sendRelayStatus(int r){
  String msg = String(r+1) + ":" + (relayState[r]?"ON":"OFF");
...
void setup(){
  ...
  if(WiFi.status()==WL_CONNECTED){
    ...
    Serial.println("\nFAILED -> AP MODE");
    WiFi.softAP("87PROJECT-SMARTRELAY","87Project.ai");
  ...
  server.on("/toggle",[]{
    ...
    sendRelayStatus(r); // update realtime ke semua client
  ...
  webSocket.onEvent([](uint8_t num, WStype_t type, uint8_t * payload, size_t length){
    // Bisa handle WS messages dari client di sini jika perlu
  });
}
